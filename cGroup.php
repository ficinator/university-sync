...
class cGroup extends cPage {
    ...
    public function getRequest() {
        if ($this->adminOfGroup) {
            ...
            while ($request = mysql_fetch_array($result)) {
                ...
                $strRet .= "ano";
                $strRet .= "</a>";
                $strRet .= "<a href=javascript:conf(\"?page=action&type=cancelRequest&requestId={$request['id']}\")>";
                $strRet .= "nie";
                $strRet .= "</a>";
                $strRet .= "<br>";
            }         
    ...
    public function groupInfo() {
        ...
        if($_GET['show'] != 'info') {
            /*$strRet .= "<div id=\"curInfo\">"
                     . "{$this->group['info']}";
            $strRet .= "</div>";   // curInfo*/
            //$strRet .= "<br><a href=\"?page=group&id={$this->group['id']}&show=settings&p=info\">";
    ...
    public function groupPhoto() {
        $strRet = "";
        $filename = "./groups/{$this->group['id']}/groupPhoto.jpg";
        if (file_exists($filename)) $path = $filename;
        else $path = "./images/ic_group_blk_120px.png";

        $strRet .= "<div id=\"groupPhoto\">";
        $strRet .= "<img src=\"{$path}\" id=\"groupPhotoImg\">";
    ...
    public function showThumbNote($noteJson, $groupId, $folderId) {
        ...
        $strRet = "";  
        $strRet .= "<div id=\"noteThumb\" title=\"{$noteJson['KeyWords'][0]}\"";
        if ($note['visual'] == '1') $strRet .= " onclick=\"window.location='?page=group&id={$groupId}&show=folder&showNote={$note['id']}'\"";
        else $strRet .= " onclick=\"window.location='?page=group&id={$groupId}&show=notes&showNote={$note['id']}'\"";
        $strRet .= ">";       
        $strRet .= "<h3>{$keyword}</h3>";      
        $noteContent = strip_tags($noteContent);
        $strRet .= "<p>".$noteContent."</p>";
        $sqlTmp = "SELECT * FROM `image` WHERE id_note = '{$note['id']}'";
        $img = mysql_num_rows(mysql_query($sqlTmp));
        if ($img != 0) $strRet .= "<span class=\"noteThumbImg\"><img src=\"./images/photo.png\" class=\"noteThumbImg\"></span>"; 
        $strRet .= "<span";
        if ($note['likes'] > 0) $strRet .= " class=\"green";
        else if ($note['likes'] < 0) $strRet .= " class=\"red";
        else $strRet .= " class=\"grey";
        $strRet .=" noteThumbRank\">";
        if ($note['likes'] > 0) $strRet .= "+";
    ...
    public function showNote($id) {
        ...
        if (!$file) $strRet .= "Nenašiel sa požadovaný príspevok.";
            ...
            $strRet .= "<div id=\"";
            if ($note['visual'] == '1') $strRet .= "noteContent";
    ...
    public function showArticlesInFolder() {
        $strRet = "";    
        $strRet .= "<div id=\"folderContent\">";
        if (1 == $this->folder['public'] || $this->folder['id_user'] == $this->user['id']) 
            $strRet .= "<button type=\"button\" class=\"btn addNoteFolderBtn btn-primary\" title=\"vytvoriť novú poznámku\" onclick=\"window.location='{$_SERVER['REQUEST_URI']}&edit=true&type=noteVisualAdd'\">+ Nová poznámka do tejto zložky"
                     . "</button>"; 
        ...
        if ($this->folder['endinfo'] != null) {
            $strRet .= "<div id=\"folderFooter\" onclick=\"window.location='?page=group&id={$this->group['id']}&show=folder&idFolder={$this->folder['id']}'\">";
    ...
    public function getBasicSearch() {
        ...
        $strRet .= "<input type=\"checkbox\" class=\"dNone\" id=\"chNotes\"";
        if ($_GET['show'] == 'notes' || $_GET['show'] == 'folder') $strRet .= " checked";
        $strRet .= "> ";
        $strRet .= "<input type=\"checkbox\" class=\"dNone\" id=\"chNews\"";
        if ($_GET['show'] == 'news' || !isset($_GET['show'])) $strRet .= " checked";
        $strRet .= "> ";
        $strRet .= "<input type=\"checkbox\" class=\"dNone\" id=\"chFiles\"";
        if ($_GET['show'] == 'files') $strRet .= " checked";
        $strRet .= "> ";
        $strRet .= "<input type=\"checkbox\" class=\"dNone\" id=\"chForum\"";
        if ($_GET['show'] == 'forum') $strRet .= " checked";
        $strRet .= "> ";
        $strRet .= "<input type=\"text\" name=\"search\" placeholder=\"čo chcete vyhľadať?\" class=\"searchBoxInput searchBox\">";
    ...
    public function folderHeader() {
        $strRet = "";
        $strRet .= "<div id=\"folderHeader\" onclick=\"window.location='?page=group&id={$this->group['id']}&show=folder&idFolder={$this->folder['id']}'\">";
        $strRet .= "<h3>{$this->folder['name']}</h3>";
        $strRet .= "</div>";
        $strRet .= "<div id=\"folderHeaderInfo\" onclick=\"window.location='?page=group&id={$this->group['id']}&show=folder&idFolder={$this->folder['id']}'\">";
            ...
            if(isset($N['show']) && $N['show'] == 'folder') {
                ...
                if (isset ($N['idFolder'])) {
                    ...
                    if (isset ($N['showNote'])) {
                        ...
                        $note = json_decode(file_get_contents($noteDb['path']), true);
                        $strRet .= "<div class=\"navigBarItem\">";
    ...
    public function showArticles() {
        ...
        if ($this->memberOfGroup || $this->adminOfGroup) {                                                                          
            $strRet .= "<button type=\"button\" class=\"btn addNoteBtn btn-primary\" title=\"vytvoriť poznámku\" onclick=\"window.location='{$_SERVER['REQUEST_URI']}&edit=true&type=noteVisualAdd'\">+ Nová poznámka</button>";
           // $strRet .= "<button type=\"button\" class=\"btn addFolderBtn btn-success\" title=\"vytvoriť novú zložku s poznámkami\" onclick=\"window.location='?page=group&show=folder&edit=true&type=folderAdd&id={$this->group['id']}'\">+ Nová zložka</button>";
        }
        if ($this->group['public'] == 1 || $this->memberOfGroup) {
            // $strRet .= "<div id=\"FAchanger\">";
            // $strRet .= "poznámky <input type=\"radio\" name=\"changer\" value=\"article\" id=\"articleChanger\" checked>";
            // $strRet .= "zložky<input type=\"radio\" name=\"changer\" value=\"folder\" id=\"folderChanger\">";
            ...
            $strRet .= "<h3>Rôzne</h3>";
            $strRet .= "<p>Všetky poznámky bez zložky</p>";
            $strRet .= "</div>";  */
            ...
            $strRet .= "Ľutujeme. Táto skupina ma uzavretý obsah.";
        }
    ...
    public function categoryTree($id = 0, $depth = 1) {
        $strRet = '';
        $sql = "SELECT * FROM `category` WHERE id_group = '{$this->group['id']}' AND id_parent = '{$id}'";
        $result = mysql_query($sql);
        $strRet .= '<ul>';
        while($category = mysql_fetch_array($result)) {
            $strRet .= "<li id=\"{$category['id']}\" class=\"selectCategories\">";
            $parent = mysql_fetch_array(mysql_query("SELECT * FROM `category` WHERE id = '{$category['id_parent']}'"));
            $strRet .= $category['name'];
            $strRet .= "</li>";
            $strRet .= $this->categoryTree($category['id'], $depth+1);
    ...
    public function showForum() {
        ...
        $strRet .= "<div id=\"forum\">";
        if ($this->memberOfGroup || $this->adminOfGroup) {    
            $strRet .= "<a href=\"?page=group&id={$this->group['id']}&edit=true&type=topicAdd&show=forum\"><button type=\"button\" class=\"btn addForumBtn btn-primary\" title=\"vytvoriť novú tému\">+ Nová téma</button></a>";
    ...
    public function showOneNew($new, $groupId, $userId) {
        ...
        $strRet .= "</div>";    // novinka
        $strRet .= cGroup::getComments($new, $groupId, $userId);
    ...
    public function showNews() {
        ...
        if($this->memberOfGroup || $this->adminOfGroup) {
            ...
            $strRet .= "<form method=\"post\" class=\"addNews\" action=\"?page=action&type=newsAdd&userId={$this->user['id']}&groupId={$this->group['id']}\" enctype=\"multipart/form-data\">";
            $strRet .= "<button type=\"submit\" title=\"vytvor novinku\" class=\"btn addNewsBtn btn-success\">Pridať</button>";
            $strRet .= "<button type=\"button\" title=\"pridaj súbor\" class=\"btn btn-xs btn-success addFileBtn\">+ súbor</button>";
            $strRet .= "<textarea required placeholder=\"Pridať novinku\" class=\"addNewsText\" name=\"content\"></textarea>";
    ...
    public function getSearch() {
        ...
        $strRet .= "poznámky a poznámky:<input type=\"checkbox\" name=\"notes\" id=\"chNotes\" class=\"searchProperties\" checked> ";
    ...
    public function getFilter() {
        ...
        $strRet .= "<button type=\"button\" class=\"btn btn-warning showCategoryTree\">filter</button>"; 
        $strRet .= "<div id=\"categoryTree\">";
        $strRet .= "<li class=\"selectCategories\">all</li>";
        $strRet .= $this->categoryTree();
        $strRet .= "<li class=\"selectCategories\">nezaradené</li>";
        $strRet .= "</div>";
        $strRet .= "Filter:";
    ...
    public function showFiles() {
        ...
        if ($this->memberOfGroup || $this->adminOfGroup) { 
            ...
            $strRet .= "Ľutujeme. Nemáte dostatčné oprávnenie na zobrazenie tohoto obsahu.";
        }
        $strRet .= "</div>";    // files
        return $strRet;
    }
    ...
    public function getButtons() {
        ...
        if (!isset($show) || $show == 'news' || $show == null || ($show != 'notes' && $show != 'folder' && $show != 'forum' && $show != 'files' && $show != 'settings')) {
            $strRet .=" active\"";
        } else $strRet .=  "\"";
        ...
        $strRet .= "onclick=\"window.location='?page=group&id={$this->group['id']}&show=folder'\">POZNÁMKY</div>";
        $strRet .= "<div class=\"menuBtn";
        ...
        $strRet .= "onclick=\"window.location='?page=group&id={$this->group['id']}&show=settings'\">INFO</div>";
